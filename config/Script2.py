...
def clean_and_save_table_ab(promptA, promptB, out_csv):
    ...
    for col in table_ab.select_dtypes(include="object").columns:
        table_ab[col] = table_ab[col].replace("nan", "").fillna("").astype(str).str.strip()
...
# ─────────────────── GRAPHVIZ ──────────────────────────────────────────────
def graph_highlight(sample, t3_path, out_graph_dir):
...
def build_table2_3(sample, context, table1, tables_dir, prompt_dir):
    ...
    for col in ["Observed Shift", "Expected Shift"]:
        t2[col] = t2[col].astype(str).str.strip().replace({"nan": ""}).str.replace(r"\.0+$", "", regex=True)
    ...
    mask = (t2["Observed Shift"] == t2["Expected Shift"]) & (t2.groupby("Biological Group")["Biological Group"].transform("size") > 1)
...
# ─────────────────── TABLE3 DIRECT ─────────────────────────────────────────
def run_table3_direct(table3_path, ctx, run_mode="interpret_graphviz"):
    """
    ...
    # Read input (do NOT copy into outputs)
    df = pd.read_csv(t3_path)
...
if __name__ == "__main__":
    ...
    if args.mode == "table3_direct":
        ...
        run_table3_direct(args.table3, args.context, args.run)
